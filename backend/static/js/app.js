...
function showPage(page) {
    ...
    else if (page === 'consents') { _refreshConsentTemplates(); loadConsents(); }
}
...
function _resetBarcodeSection() {
    document.getElementById('consent-barcode-upload-area').classList.remove('hidden');
    document.getElementById('consent-barcode-preview').classList.add('hidden');
    document.getElementById('consent-barcode-img').src = '';
}

function _loadBarcodePreview(barcodePath) {
    if (!barcodePath) {
        _resetBarcodeSection();
        return;
    }
    document.getElementById('consent-barcode-upload-area').classList.add('hidden');
    document.getElementById('consent-barcode-preview').classList.remove('hidden');
    document.getElementById('consent-barcode-img').src = barcodePath;
}

function uploadBarcodeImage(input) {
    if (!input.files || input.files.length === 0) return;
    if (!_selectedConsentId) { alert('请先保存同意书'); input.value = ''; return; }
    ...
    // 显示上传中状态
    document.getElementById('consent-barcode-upload-area').innerHTML = '<p class="text-sm text-blue-500">上传中...</p>';

    fetch('/api/consents/' + _selectedConsentId + '/barcode', {
        method: 'POST',
        body: formData
    }).then(function(res) { return res.json(); }).then(function(res) {
        // 恢复上传区域HTML
        document.getElementById('consent-barcode-upload-area').innerHTML =
        ...
        if (res.code === 200) {
            _loadBarcodePreview(res.data.barcode_image_path);
        } else {
...
function _refreshConsentTemplates() {
    ...
    api('/api/consents/templates/refresh', { method: 'POST' }).then(function(res) {
        if (res.code === 200) {
            ...
            _consentTemplateCache = {};
            console.log('[Consents] 模板数据已刷新');
        }
    }).catch(function(e) {
        console.log('[Consents] 模板刷新失败:', e);
    });
}

function toggleConsentNewMenu() {
    var menu = document.getElementById('consent-new-menu');
...
function showConsentForm(docType) {
    if (!docType) docType = '种植手术';
    ...
    // 加载模板
    _loadConsentTemplate(docType, function(template) {
        _currentFieldSchema = template.field_schema || [];
        document.getElementById('consent-doc-type').value = docType;
        document.getElementById('consent-form-title').textContent = template.title || docType + '知情同意书';
        ...
        document.getElementById('consent-allergy-no').checked = false;
        document.getElementById('consent-allergy-yes').checked = false;
        document.getElementById('consent-allergy-detail').value = '';
        document.getElementById('consent-allergy-detail').classList.add('hidden');
        document.getElementById('consent-guardian-relation').value = '';
        document.getElementById('consent-complete-btn').classList.add('hidden');

        // 渲染动态字段
        _renderConsentFields(_currentFieldSchema);
        // 渲染条款
...
function _loadConsentTemplate(docType, callback) {
    if (_consentTemplateCache[docType]) {
        callback(_consentTemplateCache[docType]);
        return;
    }
    api('/api/consents/templates/' + encodeURIComponent(docType)).then(function(res) {
        if (res.code === 200 && res.data) {
            _consentTemplateCache[docType] = res.data;
            callback(res.data);
        } else {
            // 使用默认模板
            callback({title: docType + '知情同意书', clauses: [], field_schema: []});
        }
    }).catch(function() {
        callback({title: docType + '知情同意书', clauses: [], field_schema: []});
    });
...
function _renderConsentFields(fields) {
    var container = document.getElementById('consent-fields-container');
    if (!container) return;
    if (!fields || fields.length === 0) {
        container.innerHTML = '<div class="col-span-2 text-sm text-gray-400">该类型无需额外字段</div>';
        return;
    }
    container.innerHTML = fields.map(function(f) {
        ...
        var inputHtml = '';
        if (f.type === 'text') {
            inputHtml = '<input type="text" id="field-' + f.name + '" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" placeholder="">';
        } else if (f.type === 'number') {
            inputHtml = '<input type="number" id="field-' + f.name + '" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="' + (f.default || '') + '" min="1">';
        } else if (f.type === 'tooth_selector') {
            inputHtml = '<div class="flex items-center gap-2"><input type="text" id="field-' + f.name + '" class="flex-1 px-2 py-1 border border-gray-300 rounded text-sm bg-gray-50" readonly placeholder="点击选择牙位"><button onclick="openToothSelector(' + "'" + f.name + "'" + ')" class="px-2 py-1 text-xs bg-blue-50 text-blue-600 rounded hover:bg-blue-100">选择</button></div>';
        } else if (f.type === 'yesno') {
            inputHtml = '<div class="flex items-center gap-3"><label class="text-sm"><input type="radio" name="field-' + f.name + '" value="yes"> 有</label><label class="text-sm"><input type="radio" name="field-' + f.name + '" value="no" checked> 无</label></div>';
        } else if (f.type === 'handwrite') {
            inputHtml = '<div id="field-' + f.name + '-preview" class="h-16 border rounded bg-gray-50 flex items-center justify-center text-gray-400 text-sm cursor-pointer hover:bg-gray-100" onclick="openHandwritePad(' + "'" + f.name + "'" + ')"><span id="field-' + f.name + '-text">点击手写「' + f.label + '」</span></div>';
        }
        return '<div><label class="block text-xs text-gray-500 mb-1">' + f.label + required + '</label>' + inputHtml + '</div>';
    }).join('');
}

function _renderConsentClauses(clauses) {
    var container = document.getElementById('consent-clauses-container');
    if (!container) return;
    if (!clauses || clauses.length === 0) {
        container.innerHTML = '<p class="text-gray-400">暂无条款内容</p>';
        return;
    }
    container.innerHTML = '<div class="space-y-3">' + clauses.map(function(c) {
        if (c.indexOf('{') >= 0 && c.indexOf('}') >= 0) {
            // 包含变量的条款，显示为可填写
            return '<p class="text-justify indent-8">' + c.replace(/\{(\w+)\}/g, '<u id="clause-var-$1" class="text-blue-600">________</u>') + '</p>';
        }
        return '<p class="text-justify indent-8">' + c + '</p>';
    }).join('') + '</div>';
}

function _renderHandwriteAreas(fields) {
    var container = document.getElementById('consent-handwrite-container');
    if (!container) return;
    var hwFields = (fields || []).filter(function(f) { return f.type === 'handwrite'; });
    if (hwFields.length === 0) {
        container.innerHTML = '';
        return;
    }
    // 手写区域已经在_renderConsentFields中渲染，这里不需要额外处理
    container.innerHTML = '';
}

function toggleAllergyInput() {
    var detail = document.getElementById('consent-allergy-detail');
    var yesChecked = document.getElementById('consent-allergy-yes').checked;
    if (yesChecked) {
...
function openHandwritePad(fieldName) {
    if (!_selectedConsentId) { alert('请先保存同意书'); return; }
    _currentSignType = 'handwrite:' + fieldName;
    var fieldDef = (_currentFieldSchema || []).find(function(f) { return f.name === fieldName; });
    var title = fieldDef ? fieldDef.label : fieldName;
    document.getElementById('sign-pad-title').textContent = '手写：' + title;
...
function _collectExtraFields() {
    var extra = {};
    (_currentFieldSchema || []).forEach(function(f) {
        if (f.type === 'yesno') {
            var radios = document.getElementsByName('field-' + f.name);
            for (var i = 0; i < radios.length; i++) {
                if (radios[i].checked) { extra[f.name] = radios[i].value; break; }
            }
        } else if (f.type === 'handwrite') {
            // 手写内容路径在extra中通过handwrite API保存
        } else if (f.type === 'tooth_selector') {
            var el = document.getElementById('field-' + f.name);
            extra[f.name] = el ? el.value : '';
        } else {
            var el = document.getElementById('field-' + f.name);
...
function _getAllergyHistory() {
    var yesChecked = document.getElementById('consent-allergy-yes').checked;
    if (!yesChecked) return '无';
    var detail = document.getElementById('consent-allergy-detail').value.trim();
    return detail ? '有：' + detail : '有';
}
...
function saveConsentDraft() {
    var data = {
        ...
        patient_id: null,
        allergy_history: _getAllergyHistory(),
        tooth_positions: _selectedTeeth,
        extra_fields: _collectExtraFields()
    };
    ...
    api(url, { method: method, body: JSON.stringify(data) }).then(function(res) {
        if (res.code === 200) {
            if (!_selectedConsentId && res.data && res.data.id) {
                ...
                document.getElementById('consent-no').value = res.data.doc_no;
                document.getElementById('consent-display-no').textContent = res.data.doc_no;
            }
            alert('保存成功');
            loadConsents();
        } else { alert(res.message || '保存失败'); }
...
function editConsent(id) {
    api('/api/consents/' + id).then(function(res) {
        ...
        _currentDocType = r.doc_type || '种植手术';

        document.getElementById('consent-list-view').classList.add('hidden');
        document.getElementById('consent-form-view').classList.remove('hidden');

        _loadConsentTemplate(_currentDocType, function(template) {
            _currentFieldSchema = template.field_schema || [];
            document.getElementById('consent-doc-type').value = _currentDocType;
            document.getElementById('consent-form-title').textContent = template.title || _currentDocType + '知情同意书';
            document.getElementById('consent-id').value = r.id;
            document.getElementById('consent-no').value = r.doc_no;
            document.getElementById('consent-display-no').textContent = r.doc_no;
            ...
            // 过敏史
            var allergy = r.allergy_history || '';
            if (allergy === '无') {
                document.getElementById('consent-allergy-no').checked = true;
                document.getElementById('consent-allergy-detail').classList.add('hidden');
            } else if (allergy.startsWith('有')) {
                document.getElementById('consent-allergy-yes').checked = true;
                document.getElementById('consent-allergy-detail').classList.remove('hidden');
                document.getElementById('consent-allergy-detail').value = allergy.replace(/^有[：:]?\s*/, '');
            }
            ...
            // 填充动态字段值
            var ef = r.extra_fields || {};
            (_currentFieldSchema || []).forEach(function(f) {
                if (f.type === 'yesno') {
                    var radios = document.getElementsByName('field-' + f.name);
                    for (var i = 0; i < radios.length; i++) {
                        if (radios[i].value === ef[f.name]) radios[i].checked = true;
                    }
                } else if (f.type === 'handwrite') {
                    if (ef[f.name + '_path']) {
                        var fname = ef[f.name + '_path'].split('/').pop();
                        var preview = document.getElementById('field-' + f.name + '-preview');
                        var text = document.getElementById('field-' + f.name + '-text');
                        if (preview && text) {
                            text.textContent = '';
                            preview.style.backgroundImage = 'url(/uploads/consents/' + r.doc_no + '/' + fname + ')';
                    ...
                    var el = document.getElementById('field-' + f.name);
                    if (el) el.value = (r.tooth_positions || []).join(', ');
                } else {
                    var el = document.getElementById('field-' + f.name);
                    if (el && ef[f.name]) el.value = ef[f.name];
                }
            });
...
function _loadSignPreview(type, path, signDate) {
    ...
    if (preview) {
        ...
        var no = document.getElementById('consent-no').value;
        preview.style.backgroundImage = 'url(/uploads/consents/' + no + '/' + fname + ')';
        preview.style.backgroundSize = 'contain';
        preview.style.backgroundRepeat = 'no-repeat';
        preview.style.backgroundPosition = 'center';
    }
...
function loadConsents(page) {
    ...
    var docType = document.getElementById('consent-type-filter') ? document.getElementById('consent-type-filter').value : '';
    ...
    api('/api/consents/?' + params.toString()).then(function(res) {
        ...
        tbody.innerHTML = items.map(function(r) {
            var hasPatientOrGuardian = r.patient_signature_path || r.guardian_signature_path;
            var hasDoctor = r.doctor_signature_path;
            var signStatus = (hasPatientOrGuardian && hasDoctor) ? '<span class="text-emerald-600 font-medium">已签名</span>' :
                (hasPatientOrGuardian || hasDoctor) ? '<span class="text-orange-500">部分签名</span>' :
            ...
            return '<tr class="hover:bg-gray-50">' +
                '<td class="px-4 py-3 font-mono text-xs text-gray-500">' + (r.doc_no || '') + '</td>' +
                '<td class="px-4 py-3"><span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">' + (r.doc_type || '') + '</span></td>' +
                ...
                '<td class="px-4 py-3 text-center text-gray-600 text-xs">' + toothStr + '</td>' +
                '<td class="px-4 py-3 text-center text-xs">' + signStatus + '</td>' +
                '<td class="px-4 py-3 text-center"><button onclick="previewConsentFromList(' + r.id + ')" class="text-blue-500 hover:text-blue-700 text-xs">预览</button></td>' +
...
function previewConsent() {
    ...
    api('/api/consents/' + _selectedConsentId).then(function(res) {
        ...
        document.getElementById('preview-display-no').textContent = r.doc_no || '________';
        ...
        document.getElementById('preview-patient-id').textContent = r.doc_no || '';
        document.getElementById('preview-allergy').textContent = r.allergy_history || '无';

        // 动态更新类型特定字段
        var ef = r.extra_fields || {};
        document.getElementById('preview-extra-container').innerHTML = _buildPreviewExtra(r.doc_type, ef, tp, toothNames, r.doc_no);

        // 加载模板条款
        _loadConsentTemplate(r.doc_type, function(template) {
            var clauses = template.clauses || [];
            var repl = _buildClauseReplacements(r, ef, tp, toothNames);
            document.getElementById('preview-clauses').innerHTML = clauses.map(function(c) {
                try { c = c.replace(/\{(\w+)\}/g, function(m, k) { return repl[k] || m; }); } catch(e) {}
                return '<p class="text-justify indent-8 mb-2">' + c + '</p>';
            }).join('');
        });

        // 签名
        _fillPreviewSign('patient', r.patient_signature_path, r.patient_sign_date, r.doc_no);
        _fillPreviewSign('guardian', r.guardian_signature_path, r.guardian_sign_date, r.doc_no);
        ...
        if (r.doc_type === '种植手术' && r.barcode_image_path) {
            ...
            document.getElementById('preview-barcode-img').src = r.barcode_image_path;
        } else {
            barcodeSection.classList.add('hidden');
            document.getElementById('preview-barcode-img').src = '';
        }
...
function _buildPreviewExtra(docType, ef, tp, toothNames, docNo) {
    var tpStr = tp.map(function(t) { return toothNames[t] || t; }).join('、');
    var html = '<div class="mb-4 p-3 bg-gray-50 rounded-lg text-sm space-y-1">';
    html += '<div><span class="text-gray-500">牙位：</span>' + (tpStr || '-') + '</div>';
    if (docType === '种植手术') {
        html += '<div><span class="text-gray-500">品牌：</span>' + (ef.implant_brand || '-') + ' | <span class="text-gray-500">型号：</span>' + (ef.implant_model || '-') + ' | <span class="text-gray-500">数目：</span>' + (ef.implant_count || '-') + '</div>';
    } else if (docType === '补牙') {
        html += '<div><span class="text-gray-500">诊断：</span>' + (ef.diagnosis || '-') + '</div>';
        if (ef.treatment_plan_path) html += '<div><span class="text-gray-500">治疗方案：</span><span class="text-emerald-600">已手写</span></div>';
    } else if (docType === '拔牙') {
        html += '<div><span class="text-gray-500">诊断：</span>' + (ef.diagnosis || '-') + '</div>';
        var historyLabels = ['拔牙史','药物及麻醉过敏史','血液病','心脏病等疾病','月经期或妊娠期','空腹','急性炎症期'];
        var historyHtml = historyLabels.map(function(lbl, idx) {
            var val = ef['has_history_' + (idx+1)];
            return lbl + ':' + (val === 'yes' ? '<span class="text-red-500">有</span>' : '<span class="text-gray-400">无</span>');
        }).join(' | ');
        html += '<div><span class="text-gray-500">病史确认：</span>' + historyHtml + '</div>';
    } else if (docType === '根管治疗') {
        html += '<div><span class="text-gray-500">NO.：</span>' + (docNo || '-') + ' | <span class="text-gray-500">IMP：</span>' + (ef.imp || '-') + '</div>';
        if (ef.chief_complaint_path) html += '<div><span class="text-gray-500">主诉：</span><span class="text-emerald-600">已手写</span></div>';
...
function _buildClauseReplacements(r, ef, tp, toothNames) {
    var tpStr = tp.map(function(t) { return toothNames[t] || t; }).join('、');
    return {
        ...
        patient_id: r.doc_no || '',
        tooth_positions: tpStr,
        implant_brand: ef.implant_brand || '',
        implant_model: ef.implant_model || '',
        implant_count: String(ef.implant_count || ''),
        diagnosis: ef.diagnosis || '',
        treatment_plan: ef.treatment_plan_path ? '(已手写)' : '(未手写)',
        allergy_history: r.allergy_history || '无',
        chief_complaint: ef.chief_complaint_path ? '(已手写)' : '(未手写)',
        pe: ef.pe_path ? '(已手写)' : '(未手写)',
        imp: ef.imp || '',
        patient_statement: ef.patient_statement_path ? '(已手写)' : '(未手写)',
        doctor_statement: ef.doctor_statement_path ? '(已手写)' : '(未手写)',
        has_history_1: ef.has_history_1 === 'yes' ? '有' : '无',
        has_history_2: ef.has_history_2 === 'yes' ? '有' : '无',
        has_history_3: ef.has_history_3 === 'yes' ? '有' : '无',
        has_history_4: ef.has_history_4 === 'yes' ? '有' : '无',
        has_history_5: ef.has_history_5 === 'yes' ? '有' : '无',
        has_history_6: ef.has_history_6 === 'yes' ? '有' : '无',
        has_history_7: ef.has_history_7 === 'yes' ? '有' : '无',
...
function _fillPreviewSign(type, path, signDate, docNo) {
    ...
    if (path) {
        var fname = path.split('/').pop();
        img.src = '/uploads/consents/' + docNo + '/' + fname;
...
var _activeToothField = null;
function openToothSelector(fieldName) {
    _activeToothField = fieldName || 'default';
    initToothSelector();
    // 恢复已选牙位
    if (_activeToothField === 'default') {
        _selectedTeeth = [];
    }
...
function _updateToothSelectionUI() {
    ...
    document.getElementById('tooth-selected-display').textContent = _selectedTeeth.map(function(t){return names[t]||t;}).join('、') || '无';
}
function confirmToothSelection() {
    var names = {11:'右上1',12:'右上2',13:'右上3',14:'右上4',15:'右上5',16:'右上6',17:'右上7',18:'右上8',21:'左上1',22:'左上2',23:'左上3',24:'左上4',25:'左上5',26:'左上6',27:'左上7',28:'左上8',31:'左下1',32:'左下2',33:'左下3',34:'左下4',35:'左下5',36:'左下6',37:'左下7',38:'左下8',41:'右下1',42:'右下2',43:'右下3',44:'右下4',45:'右下5',46:'右下6',47:'右下7',48:'右下8'};
    var displayStr = _selectedTeeth.map(function(t){return names[t]||t;}).join('、');
    if (_activeToothField && _activeToothField !== 'default') {
        var el = document.getElementById('field-' + _activeToothField);
        if (el) el.value = displayStr;
    }
    closeToothSelector();
}
...
function _initSignPadCanvas() {
    ...
    canvas.onmousedown = function(e) { _signPadDrawing = true; var rect = canvas.getBoundingClientRect(); _signPadLastX = e.clientX - rect.left; _signPadLastY = e.clientY - rect.top; };
    canvas.onmousemove = function(e) { if (!_signPadDrawing) return; var rect = canvas.getBoundingClientRect(); var x = e.clientX - rect.left, y = e.clientY - rect.top; _signPadCtx.beginPath(); _signPadCtx.moveTo(_signPadLastX, _signPadLastY); _signPadCtx.lineTo(x, y); _signPadCtx.stroke(); _signPadLastX = x; _signPadLastY = y; };
    canvas.onmouseup = function() { _signPadDrawing = false; };
    canvas.onmouseleave = function() { _signPadDrawing = false; };
    canvas.ontouchstart = function(e) { e.preventDefault(); var touch = e.touches[0]; var rect = canvas.getBoundingClientRect(); _signPadDrawing = true; _signPadLastX = touch.clientX - rect.left; _signPadLastY = touch.clientY - rect.top; };
    canvas.ontouchmove = function(e) { e.preventDefault(); if (!_signPadDrawing) return; var touch = e.touches[0]; var rect = canvas.getBoundingClientRect(); var x = touch.clientX - rect.left, y = touch.clientY - rect.top; _signPadCtx.beginPath(); _signPadCtx.moveTo(_signPadLastX, _signPadLastY); _signPadCtx.lineTo(x, y); _signPadCtx.stroke(); _signPadLastX = x; _signPadLastY = y; };
    canvas.ontouchend = function() { _signPadDrawing = false; };
}
function clearSignPad() { var c = document.getElementById('sign-pad-canvas'); if (_signPadCtx && c) _signPadCtx.clearRect(0, 0, c.width, c.height); }
function saveSignPad() {
    ...
    var hasPixel = false;
    for (var i = 3; i < imageData.length; i += 4) { if (imageData[i] > 0) { hasPixel = true; break; } }
    if (!hasPixel) { alert('请先签名'); return; }
    var base64 = canvas.toDataURL('image/png');

    // 手写区域签名
    if (_currentSignType.startsWith('handwrite:')) {
        var fieldName = _currentSignType.split(':')[1];
        api('/api/consents/' + _selectedConsentId + '/handwrite', {
            method: 'POST',
            body: JSON.stringify({ field_name: fieldName, signature: base64 })
        }).then(function(res) {
            if (res.code === 200) {
                closeSignPad();
                var preview = document.getElementById('field-' + fieldName + '-preview');
                var text = document.getElementById('field-' + fieldName + '-text');
                if (preview && text) {
                    text.textContent = '';
    ...
    if (_currentSignType === 'guardian') bodyData.guardian_relation = document.getElementById('consent-guardian-relation').value;
    api('/api/consents/' + _selectedConsentId + '/sign', { method: 'POST', body: JSON.stringify(bodyData) }).then(function(res) {
        if (res.code === 200) {
            closeSignPad();
            api('/api/consents/' + _selectedConsentId).then(function(r2) {
                if (r2.code === 200) {
                    ...
                    _checkCompleteBtn(rd);
                    if (rd.status === '已完成') alert('所有签名已完成，可以归档了');
                }
            });
        } else { alert(res.message || '保存失败'); }
    }).catch(function(e) { alert('保存失败: ' + e.message); });
